...
#include <vgc/dom/detail/facecycle.h>

#include <vgc/dom/value.h>

namespace vgc::dom::detail {

void CycleComponent::write(StreamWriter& out) const {
    using core::write;
    write(out, path_);
    if (!direction_) {
        write(out, '*');
...
void CycleComponent::read(StreamReader& in) {
    using core::readTo;
    readTo(path_, in);
    char c = -1;
    if (in.get(c)) {
...
void Cycle::write(StreamWriter& out) const {
    using core::write;
    bool first = true;
    for (const CycleComponent& component : components_) {
        if (!first) {
            write(out, ' ');
        }
        else {
...
void Cycle::read(StreamReader& in) {
    using core::readTo;
    components_.clear();
    char c = -1;
    bool got = false;
